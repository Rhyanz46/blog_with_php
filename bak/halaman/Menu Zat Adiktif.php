...
$sql = "SELECT * FROM narkoba WHERE kategori='zatadiktif';";
...
foreach ($data_narkotika as $data) {
	...
	$i++;
	echo '<button ke="Detail" nilai="Sub Menu Zat Adiktif" idnya="'.$data["id"].'" type="button" class="btn btn-md btn-danger" style="width: 13em !important;">'.$data["nama"].'</button>';
	if ($i%2==1) {
		echo "</div> <br>";
	} 
}
